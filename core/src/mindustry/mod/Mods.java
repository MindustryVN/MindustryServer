...
public class Mods implements Loadable {
    ...
    /**
     * Returns a file named 'config.json' in a special folder for the specified
     * plugin. Call this in init().
     */
    ...
    public LoadedMod importMod(Fi file) throws IOException {
        // for some reason, android likes to add colons to file names, e.g.
        // primary:ExampleJavaMod.jar, which breaks dexing
    ...
    public void loadAsync() {
        if (!mods.contains(LoadedMod::enabled))
            return;
        Time.mark();
    ...
    private void packSprites(Seq<Fi> sprites, LoadedMod mod, boolean prefix, Seq<Future<Runnable>> tasks) {
        ...
        for (Fi file : sprites) {
            ...
            if (!prefix && !Core.atlas.has(regionName)) {
                Log.warn("Sprite '@' in mod '@' attempts to override a non-existent sprite. Ignoring.", regionName,
                        mod.name);
                continue;
            ...
            tasks.add(mainExecutor.submit(() -> {
                ...
                try {
                    ...
                    // this returns a *runnable* which actually packs the resulting pixmap; this has
                    // to be done synchronously outside the method
                    return () -> {
    ...
    public void loadSync() {
        ...
        if (totalSprites > 0) {
            ...
            ObjectMap<Texture, PageType> pageTypes = ObjectMap.of(Core.atlas.find("white").texture, PageType.main,
                    Core.atlas.find("stone1").texture, PageType.environment, Core.atlas.find("clear-editor").texture,
                    PageType.editor, Core.atlas.find("whiteui").texture, PageType.ui,
            ...
            for (AtlasRegion region : Core.atlas.getRegions()) {
                ...
                if (!packer.has(type, region.name)) {
                    entries[type.ordinal()].add(
                            new RegionEntry(region.name, Core.atlas.getPixmap(region), region.splits, region.pads));
                }
            }
    ...
    private PageType getPage(Fi file) {
        ...
        return path.contains("sprites/blocks/environment") || path.contains("sprites-override/blocks/environment")
    ...
    public void load() {
        ...
        platform.getWorkshopContent(LoadedMod.class).each(candidates::add);
    ...
    private void sortMods() {
        // sort mods to make sure servers handle them properly and they appear correctly
        // in the dialog
        ...
        for (LoadedMod mod : orderedMods()) {
            ...
            for (Fi file : mod.root.list()) {
                ...
                if (file.isDirectory() && !specialFolders.contains(file.name())) {
                    file.walk(
                            f -> tree.addFile(mod.file.isDirectory() ? f.path().substring(1 + mod.file.path().length())
                                ...
                                for (Content c : m.erroredContent) {
                                    ...
                                    d.button("@details", Icon.downOpen, Styles.cleart, () -> {
                                        new Dialog("") {
                                            {
                                                setFillParent(true);
                                                cont.pane(e -> e.add(c.minfo.error).wrap().grow()
                                                        .labelAlign(Align.center, Align.left)).grow();
                                                cont.row();
        ...
        try {
            eachEnabled(mod -> {
                if (mod.root.child("scripts").exists()) {
                    ...
                    // if there's only one script file, use it (for backwards compatibility); if
                    // there isn't, use "main.js"
    ...
    public void loadContent() {
        ...
        for (LoadRun l : runs) {
            ...
            try {
                ...
                Content loaded = parser.parse(l.mod, l.file.nameWithoutExtension(), l.file.readString("UTF-8"), l.file,
                        l.type);
    ...
    /**
     * @return the mods that the client is missing. The inputted array is changed to
     *         contain the extra mods that the client has but the server doesn't.
     */
    ...
    private boolean resolve(String element, ModResolutionContext context) {
        ...
        for (final var dependency : context.dependencies.get(element)) {
            ...
            if (context.visited.contains(dependency.name) && !context.ordered.contains(dependency.name)) {
                ...
                if (!context.ordered.contains(dependency.name) && !resolve(dependency.name, context)
                        && dependency.required) {
    ...
    /**
     * Loads a mod file+meta, but does not add it to the list. Note that directories
     * can be loaded as mods.
     */
    ...
    private LoadedMod loadMod(Fi sourceFile, boolean overwrite, boolean initialize) throws Exception {
        ...
        try {
            ...
            String mainClass = meta.main == null ? camelized.toLowerCase(Locale.ROOT) + "." + camelized + "Mod"
                    : meta.main;
            ...
            // make sure the main class exists before loading it; if it doesn't just don't
            // put it there
            // if the mod is explicitly marked as java, try loading it anyway
            if ((mainFile.exists() || meta.java) && !skipModLoading()
                    && Core.settings.getBool("mod-" + baseName + "-enabled", true)
                    && Version.isAtLeast(meta.minGameVersion) && (meta.getMinMajor() >= 136 || headless)
                    && initialize) {
                if (ios) {
                ...
                if ((main.getSuperclass().getName().equals("mindustry.mod.Plugin")
                        || main.getSuperclass().getName().equals("mindustry.mod.Mod"))
    ...
    public static class LoadedMod implements Publishable, Disposable {
        ...
        /**
         * The root zip file; points to the contents of this mod. In the case of
         * folders, this is the same as the mod's file.
         */
        ...
        /**
         * Class loader for JAR mods. Null if the mod isn't loaded or this isn't a jar
         * mod.
         */
        ...
        /**
         * Some mods are known to cause issues with the game; this detects and returns
         * whether a mod is manually blacklisted.
         */
        ...
        public String toString() {
            return "LoadedMod{" + "file=" + file + ", root=" + root + ", name='" + name + '\'' + '}';
        }
    }
    ...
    public static class ModMeta {
        ...
        /**
         * Hidden mods are only server-side or client-side, and do not support adding
         * new content.
         */
        public boolean hidden;
        /**
         * If true, this mod should be loaded as a Java class mod. This is technically
         * optional, but highly recommended.
         */
        public boolean java;
        /**
         * If true, -outline regions for units are kept when packing. Only use if you
         * know exactly what you are doing.
         */
        public boolean keepOutlines;
        /**
         * To rescale textures with a different size. Represents the size in pixels of
         * the sprite of a 1x1 block.
         */
        ...
        public String shortDescription() {
            return Strings.truncate(subtitle == null
                    ? (description == null || description.length() > maxModSubtitleLength ? "" : description)
                    : subtitle, maxModSubtitleLength, "...");
        }
        ...
        @Override
        public String toString() {
            return "ModMeta{" + "name='" + name + '\'' + ", minGameVersion='" + minGameVersion + '\''
                    + ", displayName='" + displayName + '\'' + ", author='" + author + '\'' + ", description='"
                    + description + '\'' + ", subtitle='" + subtitle + '\'' + ", version='" + version + '\''
                    + ", main='" + main + '\'' + ", repo='" + repo + '\'' + ", dependencies=" + dependencies
                    + ", softDependencies=" + softDependencies + ", hidden=" + hidden + ", java=" + java
                    + ", keepOutlines=" + keepOutlines + ", texturescale=" + texturescale + ", pregenerated="
                    + pregenerated + '}';
        }
    }
    ...
    public enum ModState {
        enabled, contentErrors, missingDependencies, incompleteDependencies, circularDependencies, unsupported,
        disabled,
    }
